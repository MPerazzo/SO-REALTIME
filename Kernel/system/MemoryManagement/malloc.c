...
void * malloc(uint64_t size) {
	...
	if ( (uint64_t)prev->chunk->start + size > UH_MEMORY_END ) {
		return NULL;
	}
...
void init_uheap() {
	...
	chunk = (mem_chunk_t*)(long int)(UH_MEMORY_START);
	memset(chunk, 0, size);
	...
	allocated = (void*)(UH_MEMORY_START + sizeof(mem_chunk_t));
	allocated->next = NULL;
